...
description: Learn how to use the ADE extensibility model to build and utilize custom Pulumi images within your environment definitions for deployment environments.
...
Depending on which programming language(s) you intend to use for Pulumi programs, you may need to intall one or more corresponding runtime(s). The Python runtime is already available in the base image.

Here is an example of installing Node.js and TypeScript:
...
To login in to Pulumi Cloud instead, set your Pulumi access token as an environment variable, and run the following commands:
